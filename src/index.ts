...
/**
 * Class that has all the mouse in canvas data and methods
 */
class MouseSubject {
	...
	constructor(canvas) {
		...
		canvas.addEventListener("mousemove", (e: MouseEvent) => {
			...
			if (this.mousedown) {
				[this.x, this.y] = MouseSubject.convertEventCoordinates(e, screenLimit);
				this.notifyObservers();
			}
		});
	}
	...
	/**
	 * Converts x and y data so that boundaries go from -1 to 1
	 * @param e MouseEvent
	 */
	static convertEventCoordinates(e: MouseEvent, canvasWidth: number) {
		const screenLimit = canvasWidth;
		const x = -((2 * e.offsetX) / screenLimit - 1);
		const y = -((2 * e.offsetY) / screenLimit - 1);
		return [x, y];
	}
